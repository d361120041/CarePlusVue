      ...
      <template v-if="isUserLogin">
        <div class="user-info">
          ...
          <router-link to="/user-center/profile" class="user-icon-wrapper">
            ...
            <img v-else src="@/assets/defaultImg/default-avatar.jpg" alt="預設頭像" class="user-icon" title="編輯個人資料" />
          </router-link>
...
/** 1. 建立四個 template ref 來綁定你的角色圖 */
const caregiverChar = ref(null)
const courseChar = ref(null)
const newsChar = ref(null)
const socialChar = ref(null)
const charMap = {
  caregiver: caregiverChar,
  course: courseChar,
  news: newsChar,
  social: socialChar,
}
...
const showCharacter = (key) => {
  const el = charMap[key]?.value
  if (el) el.style.opacity = '1'
}
const hideCharacter = (key) => {
  const el = charMap[key]?.value
